...
export const EXTRA_TOOLS = [
  ...
  { name: 'plan_task', description: '为复杂任务创建执行计划。当任务需要多个步骤时，先调用此工具拆解子任务，然后逐一执行。', parameters: { type: 'object', properties: { goal: { type: 'string', description: '任务总目标' }, subtasks: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, title: { type: 'string' }, priority: { type: 'string', enum: ['high','medium','low'] } }, required: ['id','title'] } } }, required: ['goal','subtasks'] }, parallelSafe: false, riskLevel: 'low' },
  ...
  { name: 'spawn_subagent', description: 'Spawn a sub-agent for focused investigation', parameters: { type: 'object', properties: { type: { type: 'string', enum: ['explore','research','review','security-review','battle'] }, task: { type: 'string' }, numAgents: { type: 'number', description: 'Number of competing agents (battle mode only, default 3)' } }, required: ['type','task'] }, parallelSafe: false, riskLevel: 'medium' },
