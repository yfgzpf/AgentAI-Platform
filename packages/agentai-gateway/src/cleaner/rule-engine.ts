...
/** Regex metacharacters that must be escaped before injecting into a RegExp. */
const REGEX_META = /[.+^$|()[\]\\]/g;
 ...
 * 转义正则元字符,避免误匹配
 *
 * 跨平台: Windows 路径使用反斜杠分隔, 而 glob 习惯用正斜杠。
 * 将 pattern 与 target 都归一化为正斜杠后再匹配, 保证在 win32/macOS/linux 行为一致。
 */
function globMatch(pattern: string, target: string, ctx: MatchContext): boolean {
    const norm = (s: string) => s.replace(/\\/g, '/');
    const escaped = norm(expandTemplate(pattern, ctx)).replace(REGEX_META, (m) => '\\' + m);
    const re = new RegExp('^' + escaped.replace(/\*\*/g, '.*').replace(/\*/g, '[^/]*') + '$');
    return re.test(norm(target));
}
...
export function matchRule(rules: Rule[], file: FileMeta, ctx?: Partial<MatchContext>): Rule | null {
    ...
    for (const r of rules) {
        // 跳过禁用的规则 (rules.json 中 enabled: false)
...
/** 监听规则文件变化,变化时重新加载; 返回 dispose 函数用于停止监听 */
