...
export const MODELS: ModelMeta[] = [
  // --- AgentAI (主模型, 免费+付费混合) ---
  {
    ...
    complexityRange: ['ultraSimple', 'hard'],
    maxContext: 256_000,
    costPer1kInput: 0,
    costPer1kOutput: 0,
    isFree: true,
  ...
  // --- 豆包/火山引擎 Doubao (付费) ---
  {
    ...
    provider: 'doubao',
    subModel: 'doubao-seed-2.0-pro-250728',
    complexityRange: ['simple', 'complex'],
    maxContext: 128_000,
    costPer1kInput: 0,
    costPer1kOutput: 0,
...
export function computeCapabilities(meta: ModelMeta | undefined): ModelCapability {
  if (!meta) {
    // 未知模型默认 guided (而非 supervised): 不在注册表中的模型
    // 大多是 256K+ 上下文的好模型 (sensenova/longcat/dxnt/superapi 等),
    // 只是尚未录入注册表, 不应被当作弱模型
    return {
      reasoning: 0.5, context: 0.8, speed: 0.6, vision: 0,
      toolCall: 0.5, costScore: 0.8, overall: 0.5, tier: 'guided',
    };
  }
