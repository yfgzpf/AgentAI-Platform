/**
 * IndustryEngine v2 — 多行业知识引擎
 * =================================================================
 * ⚠️ 架构注意 (2026-07-19 审计): 本文件 ~690 行硬编码行业业务规则
 *   (美缝计算器、空间报价模板、行业主题色、技能定义等)。
 *   当前为 TypeScript 内联数据, 非 AI 驱动生成。
 *
 * 改进方向:
 *   - 将静态数据移入 JSON/YAML 配置文件 (industry-data/)
 *   - 行业技能 handler 改为 AI 动态生成而非硬编码占位符
 *   - 定价表/工程项 应支持用户自定义覆盖
 *
 * 当前状态: 活跃使用中, 被 index.ts + routes/profile.ts + agentai-loop.ts 引用。
