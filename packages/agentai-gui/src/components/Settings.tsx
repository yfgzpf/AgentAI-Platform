...
const MODEL_TEMPLATES: ModelTemplate[] = [
  ...
  { id: 'moonshot', label: '月之暗面 Moonshot', baseURL: 'https://api.moonshot.cn/v1', models: ['moonshot-v1-8k', 'moonshot-v1-32k', 'moonshot-v1-128k'], docsUrl: 'https://platform.moonshot.cn/console/api-keys', color: '#6466F1', contextWindow: 128000 },
  ...
  { id: 'minimax', label: 'MiniMax', baseURL: 'https://api.minimax.chat/v1', models: ['MiniMax-M3-Flash', 'MiniMax-M3-Turbo'], docsUrl: 'https://platform.minimaxi.com', color: '#06B6D4', contextWindow: 128000 },
  { id: 'anthropic', label: 'Anthropic Claude', baseURL: 'https://api.anthropic.com/v1', models: ['claude-3-opus-20240229', 'claude-3-sonnet-20240229', 'claude-3-haiku-20240307'], docsUrl: 'https://console.anthropic.com/', color: '#D97706', contextWindow: 200000 },
  ...
  { id: 'nvidia', label: 'NVIDIA NIM (免费额度)', baseURL: 'https://integrate.api.nvidia.com/v1', models: ['deepseek-ai/deepseek-v4-flash', 'deepseek-ai/deepseek-v4-pro', 'z-ai/glm-5.2', 'nvidia/nemotron-3-ultra-550b-a55b'], docsUrl: 'https://build.nvidia.com/', color: '#76B900', contextWindow: 1000000 },
];
...
// ===== 免费模型内置列表 =====
const FREE_MODELS = [
  { id: 'agentai', label: 'ALTES | 岐黄 Free (Flash)', envVar: 'AGENTAI_API_KEY', baseURL: 'https://apihub.agnes-ai.com/v1', color: '#4F46E5' },
...
export const Settings: React.FC = () => {
  ...
  // 自定义模型
  const [newModel, setNewModel] = useState({ label: '', baseURL: '', apiKey: '', modelName: '', color: '#6366F1' });
  ...
  const testConnection = async (baseURL: string, apiKey: string, modelName: string, testId: string) => {
    setTestingId(testId);
    try {
      // 通过 Gateway 代理测试, 传递 apiKey 参数（测试时密钥可能还未保存到 process.env）
      const resp = await fetch(httpUrl() + '/v1/settings/keys/test', {
        method: 'POST',
  ...
  const configureTemplate = async (template: ModelTemplate) => {
    ...
    const saved = await saveApiKey(template.id, templateApiKey);
    if (!saved) {
  ...
  const doAddCustomModel = async () => {
    ...
    setNewModel({ label: '', baseURL: '', apiKey: '', modelName: '', color: '#6366F1' });
    // 重新加载密钥状态
    loadAllKeyStatus();
  };
  ...
  return (
    <div style={{ padding: 24, color: '#fff', maxWidth: 900, margin: '0 auto' }}>
      <h2><SettingOutlined /> Atlas 设置</h2>
            ...
            children: (
              <Card>
                <Space direction="vertical" style={{ width: '100%' }} size={16}>
                  <Divider style={{ fontSize: 12, color: '#888', margin: 0 }}>通知提醒</Divider>
                  {/* ... desktop notify ... */}
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 13, color: '#ddd' }}><BellOutlined /> 桌面通知</div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 13, color: '#ddd' }}><SoundOutlined /> 提示音</div>
                  ...
                  <Divider style={{ fontSize: 12, color: '#888', margin: 0 }}>语音</Divider>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 13, color: '#ddd' }}><SoundOutlined /> AI 回复语音播报</div>
                  ...
                  {ttsEnabled && (
                    <div style={{ marginTop: 12, marginLeft: 24 }}>
                      <div style={{ fontSize: 12, color: '#aaa', marginBottom: 8 }}>选择音色</div>
                      <VoiceSelector />
                    </div>
                  )}

                  <Divider style={{ fontSize: 12, color: '#888', margin: 0 }}>自动化</Divider>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 13, color: '#ddd' }}>🤖 自动恢复</div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 13, color: '#ddd' }}>💬 追问卡片</div>
                  ...
                  <Divider style={{ fontSize: 12, color: '#888', margin: 0 }}>模式设置</Divider>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 13, color: '#ddd' }}>🔮 默认模式</div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 13, color: '#ddd' }}>💡 智能模式推荐</div>
                  ...
                  <Divider style={{ fontSize: 12, color: '#888', margin: 0 }}>存储</Divider>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 13, color: '#ddd' }}>💾 自动记忆</div>
            ...
            children: (
              <Card>
                <Space direction="vertical" style={{ width: '100%' }} size={20}>
                  ...
                  {/* ===== 免费模型区域 ===== */}
                  <div>
                    <Divider orientation="left" style={{ fontSize: 14, color: '#4ADE80', margin: 0 }}>
                      <ThunderboltOutlined style={{ color: '#4ADE80', marginRight: 6 }} />
                      免费模型 (内置, 无需配置)
                    </Divider>
                        ...
                        return (
                          <div key={m.id} style={{
                            ...
                            <Space>
                              ...
                              <Text strong style={{ color: '#fff' }}>{m.label}</Text>
                              <Tag color="green">免费</Tag>
                              <code style={{ fontSize: 10, color: '#888' }}>{m.baseURL}</code>
                            </Space>
                            <Space>
                              {status?.ok
                              ...
                              {editingProvider === m.id ? (
                                <Space.Compact>
                                  ...
                                  <Button size="small" type="primary" icon={<SaveOutlined />} onClick={async () => {
                                    if (await saveApiKey(m.id, editApiKey)) setEditingProvider(null);
                                  }}>保存</Button>
                    ...
                    <div style={{ fontSize: 11, color: '#888', marginTop: 8, paddingLeft: 4 }}>
                  ...
                  <div>
                    <Divider orientation="left" style={{ fontSize: 14, color: '#8B5CF6', margin: 0 }}>
                      <ApiOutlined style={{ color: '#8B5CF6', marginRight: 6 }} />
                      ...
                      <Space.Compact style={{ width: '100%' }}>
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            if (ok) {
                              await saveApiKey('superapi', superApiKey);
                    ...
                    <div
                      style={{ fontSize: 12, color: '#aaa', marginBottom: 8, cursor: 'pointer', userSelect: 'none' }}
                  ...
                  {/* ===== 商用模型区域 (备选接入方案) ===== */}
                  <div>
                    <Divider orientation="left" style={{ fontSize: 14, color: '#F59E0B', margin: 0 }}>
                          ...
                          label: (
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                              <Space>
                                ...
                                <Text strong style={{ color: '#fff' }}>{t.label}</Text>
                                {['sensenova', 'longcat', 'nvidia'].includes(t.id)
                                  ? <Tag color="green">免费额度</Tag>
                                  : <Tag color="orange">付费</Tag>
                                }
                                <code style={{ fontSize: 10, color: '#888' }}>{t.baseURL}</code>
                              </Space>
                              <Space>
                                {status?.ok
                              ...
                              {configuringTemplate === t.id ? (
                                ...
                                <Space>
                                  ...
                                  <Button
                                    ...
                                    onClick={() => testConnection(t.baseURL, localStorage.getItem(`${t.id.toUpperCase()}_API_KEY`) || '', t.models[0], t.id)}
                  ...
                  {/* ===== 自定义模型区域 ===== */}
                  <div>
                    <Divider orientation="left" style={{ fontSize: 14, color: '#888', margin: 0 }}>
                    ...
                    {models.filter(m => !m.isBuiltIn).length > 0 && (
                      <div style={{ marginTop: 12 }}>
                        <div style={{ fontSize: 12, color: '#888', marginBottom: 8 }}>已添加的模型:</div>
                        {models.filter(m => !m.isBuiltIn).map((m, idx) => (
                          <div key={m.id} style={{
                            display: 'flex', alignItems: 'center', gap: 8,
                            padding: '8px 12px', borderRadius: 8,
                            ...
                            <Button
                              size="small"
                              icon={<CheckCircleOutlined />}
                              onClick={() => testConnection(m.baseURL, localStorage.getItem(m.apiKeyEnv) || '', m.models?.[0] || m.label, `custom-${idx}`)}
                              loading={testingId === `custom-${idx}`}
                              style={{ fontSize: 10 }}
                            >测试</Button>
                            <Button size="small" danger icon={<DeleteOutlined />} onClick={() => { removeModel(m.id); message.info(`已移除「${m.label}」`); }} style={{ fontSize: 10 }} />
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div style={{ padding: '16px', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--panel)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Space>
                        ...
                        <Tag color="orange">付费</Tag>
                        <span style={{ fontSize: 12, color: '#888' }}>强力推理, 适合代码审查/架构/安全</span>
                      </Space>
                      <Space>
                        {allKeyStatus.deepseek?.ok
                          ...
                          : <Button size="small" type="primary" onClick={async () => {
                              ...
                              if (!key?.trim()) return;
                              await saveApiKey('deepseek', key.trim());
                            }}>一键启用</Button>
                        }
                      </Space>
                    </div>
              ...
              <Card>
                <Space direction="vertical" style={{ width: '100%' }} size={16}>
                  <div>
                    ...
                    <Space>
                      <Button type={active === 'agentai' ? 'primary' : 'default'} onClick={() => setActive('agentai')}>Atlas (自研框架)</Button>
              ...
              <Card>
                <Space direction="vertical" style={{ width: '100%' }} size={16}>
                  <Space>
                    ...
                    {qqStatus.sessionId && <span style={{ color: '#888', fontSize: 12 }}>Session: {qqStatus.sessionId}</span>}
                  </Space>
                  {qqStatus.online && (
            ...
            children: (
              <Card>
                <Descriptions column={1} bordered>
                  <Descriptions.Item label="项目">Atlas v0.4.0-alpha</Descriptions.Item>
                  ...
                  <Descriptions.Item label="3 框架参照">自研 Atlas + Hermes 多渠道</Descriptions.Item>
                </Descriptions>
              </Card>
            ),
          },
