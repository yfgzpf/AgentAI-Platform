  ...
  const SessionItem: React.FC<{ s: any }> = ({ s }) => {
    ...
    return (
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 2, flexShrink: 0, opacity: isActive ? 1 : 0, transition: 'opacity 0.15s' }}>
          <Popconfirm title="删除此会话?" onConfirm={(e) => { e?.stopPropagation(); handleDelete(s.id); }} onCancel={(e) => e?.stopPropagation()}>
            <button onClick={(e) => e.stopPropagation()}
              style={{ padding: 2, border: 'none', background: 'transparent', color: 'var(--muted-2)', cursor: 'pointer' }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--danger)'; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = 'var(--muted-2)'; }}
            >
              <SearchOutlined style={{ fontSize: 11, display: 'none' }} /> {/* spacer */}
              ✕
            </button>
          </Popconfirm>
        </div>
