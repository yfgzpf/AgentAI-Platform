<p align="center">
  <img src="./packages/agentai-gui/public/favicon.svg" alt="ATLAS" width="120">
</p>
...
| **先诊断后治疗** | ❌ 直接开干，经常跑偏 | ✅ **任务感知 → 诊断 → 计划 → 执行** |
...
中医诊疗讲究**"望闻问切"四诊合参**，ALTES 将其转化为 AI 的决策链路：
...
├──────────┼──────────┼──────────┼──────────┼──────────┼─────────────┤
│  ALTES   │ 任务感知 │ 缺口分析 │ 探询追问 │ 结构化   │ 方案→执行   │
│  映射    │          │          │          │ 诊断     │ →调方       │
├──────────┼──────────┼──────────┼──────────┼──────────┼─────────────┤
...
ALTES 不追求"一招鲜"，而是根据**任务复杂度**和**风险等级**选择不同治法：

| 任务特征 | 对应中医治法 | ALTES 策略 |
|---------|-------------|-----------|
| 简单明确 | **单刀直入** | 直接执行，快速响应 |
...
| 典籍 | 核心思想 | 在 ALTES 中的体现 |
|------|---------|------------------|
| **《黄帝内经》** | "上工治未病" | 任务感知，提前发现风险 |
| **《伤寒论》** | "随证治之，辨证论治" | 根据任务类型选择不同策略 |
| **《千金方》** | "大医精诚" | 自我验证，追求精准 |
| **《温病条辨》** | "三焦辨证，分阶段治疗" | 复杂任务拆解，分步执行 |

> **注**：岐黄之术的借鉴是**理念层面**的——我们学习中医"先诊后治"的思维模式，而非具体医学知识。ALTES 是一个**工程系统**，不是医疗 AI。
...
ALTES 采用**薄包装架构**——不重复造轮子，而是把优秀的开源能力串成链路：
...
- [x] 行业洞察自主积累 + RAG 知识库
- [x] 系统自管理（健康自检 + 自动修复）
- [x] 智能模型路由（5 维评分 + 熔断切换 + 成本守卫）
- [x] Token 4 重压缩（节省 70-85%）
- [x] 自我进化系统（evolve_prompt + create_tool）
- [x] 安全审批流（风险分级 + 信任白名单 + 审计日志）
- [x] 自动验证闭环（写→验→修→再验）
- [x] 多模态（图片/TTS/音乐/3D/SVG 图表/文档解析）
- [x] Web GUI（15 个功能页面）
- [x] Goal 模式（目标驱动 + 断点续跑）
- [x] 智能追问（歧义检测 + 置信度分流）
